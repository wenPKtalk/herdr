...
"#,
    ...
    #[test]
    fn semantic_client_down_moves_keybind_help_selection() {
        let mut server = test_headless_server();
        server.app.state.mode = crate::app::Mode::KeybindHelp;
        server.clients.insert(
            1,
        ...
        // Fork behavior: keybind help is an interactive action picker, so Down
        // advances the actionable-row selection (scroll only follows when the
        // selected row would leave the viewport) rather than scrolling directly.
